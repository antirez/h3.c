...
int h3_cuda_probe(h3_device_info *info, char *error, size_t error_size) {
    ...
    if (info) {
        ...
        info->unified_memory = (prop.unifiedMemory ? 1 : 0);
    }
    return 1;
}
...
static void h3_cuda_seterr(h3_gpu *gpu) {
    if (gpu) snprintf(gpu->error, sizeof(gpu->error), "%s", H3_CUDA_ERR);
}

int h3_gpu_is_m5(const h3_gpu *gpu) { h3_cuda_seterr(gpu); return 0; }
...
h3_gpu_tensor * h3_gpu_tensor_new_f32(h3_gpu *gpu, size_t elements) { h3_cuda_seterr(gpu); return NULL; }
h3_gpu_tensor * h3_gpu_tensor_new_bf16(h3_gpu *gpu, size_t elements) { h3_cuda_seterr(gpu); return NULL; }
h3_gpu_tensor * h3_gpu_tensor_new_i8(h3_gpu *gpu, size_t elements) { h3_cuda_seterr(gpu); return NULL; }
h3_gpu_tensor * h3_gpu_tensor_from_f32(h3_gpu *gpu, const float *values,
                                      size_t elements) { h3_cuda_seterr(gpu); return NULL; }
...
h3_gpu_tensor * h3_gpu_tensor_load_bf16(h3_gpu *gpu, const char *path,
                                       uint64_t file_offset, size_t elements) { h3_cuda_seterr(gpu); return NULL; }
...
int h3_gpu_tensor_read_file_bf16(h3_gpu_tensor *tensor, const char *path,
                                 ...
                                 char *error, size_t error_size) { return 0; }
int h3_gpu_tensor_stream_file_bf16(h3_gpu_tensor *tensor, const char *path,
                                   uint64_t file_offset, size_t elements,
                                   char *error, size_t error_size) { return 0; }
void h3_gpu_tensor_free(h3_gpu_tensor *tensor) { }
size_t h3_gpu_tensor_elements(const h3_gpu_tensor *tensor) { return 0; }
h3_gpu_dtype h3_gpu_tensor_dtype(const h3_gpu_tensor *tensor) { return 0; }
int h3_gpu_tensor_read_f32(const h3_gpu_tensor *tensor, float *values,
                           size_t elements) { return 0; }
...
int h3_gpu_tensor_write_f32_range(h3_gpu_tensor *tensor,
                                  ...
                                  const float *values, size_t elements) { return 0; }
...
int h3_gpu_tensor_write_bf16_range(h3_gpu_tensor *tensor,
                                   size_t destination_offset,
                                   const uint16_t *values, size_t elements) { return 0; }
int h3_gpu_begin(h3_gpu *gpu) { h3_cuda_seterr(gpu); return 0; }
...
int h3_gpu_linear_f32(h3_gpu *gpu, h3_gpu_tensor *output,
                      ...
                      uint32_t input_dim, uint32_t output_dim) { h3_cuda_seterr(gpu); return 0; }
...
int h3_gpu_silu_f32(h3_gpu *gpu, h3_gpu_tensor *output,
                    const h3_gpu_tensor *input, uint32_t elements) { h3_cuda_seterr(gpu); return 0; }
...
int h3_gpu_rms_norm_f32(h3_gpu *gpu, h3_gpu_tensor *output,
                        ...
                        uint32_t width, float epsilon) { h3_cuda_seterr(gpu); return 0; }
int h3_gpu_adaln_f32(h3_gpu *gpu, h3_gpu_tensor *output,
                     ...
                     uint32_t scale_slot, float epsilon) { h3_cuda_seterr(gpu); return 0; }
int h3_gpu_gate_f32(h3_gpu *gpu, h3_gpu_tensor *output,
                    const h3_gpu_tensor *residual,
                    const h3_gpu_tensor *branch,
                    const h3_gpu_tensor *modulation,
                    const h3_gpu_tensor *row_map, uint32_t rows,
                    uint32_t width, uint32_t slots, uint32_t gate_slot) { h3_cuda_seterr(gpu); return 0; }
int h3_gpu_qkv_rope_f32(h3_gpu *gpu, h3_gpu_tensor *query,
                        ...
                        uint32_t rope_half, float epsilon) { h3_cuda_seterr(gpu); return 0; }
int h3_gpu_sdpa_f32(h3_gpu *gpu, h3_gpu_tensor *output,
                    const h3_gpu_tensor *query, const h3_gpu_tensor *key,
                    const h3_gpu_tensor *value, uint32_t sequence,
                    uint32_t heads, uint32_t head_dim, float scale) { h3_cuda_seterr(gpu); return 0; }
...
int h3_gpu_video_qkv_rope_f32(h3_gpu *gpu, h3_gpu_tensor *query,
                              ...
                              float epsilon) { h3_cuda_seterr(gpu); return 0; }
int h3_gpu_conv1d_f32(h3_gpu *gpu, h3_gpu_tensor *output,
                      ...
                      uint32_t padding, uint32_t dilation) { h3_cuda_seterr(gpu); return 0; }
int h3_gpu_conv1d_stride_f32(h3_gpu *gpu, h3_gpu_tensor *output,
                      ...
                      uint32_t dilation) { h3_cuda_seterr(gpu); return 0; }
int h3_gpu_conv_transpose1d_f32(
                      ...
                      uint32_t length, uint32_t input_channels,
                      uint32_t output_channels, uint32_t kernel,
                      uint32_t stride, uint32_t padding) { h3_cuda_seterr(gpu); return 0; }
int h3_gpu_weight_norm_f32(h3_gpu *gpu, h3_gpu_tensor *output,
                           const h3_gpu_tensor *vector,
                           const h3_gpu_tensor *magnitude,
                           uint32_t outer, uint32_t inner) { h3_cuda_seterr(gpu); return 0; }
int h3_gpu_add_scaled_f32(h3_gpu *gpu, h3_gpu_tensor *output,
                          ...
                          float right_scale, uint32_t elements) { h3_cuda_seterr(gpu); return 0; }
int h3_gpu_alias_free_snake_f32(
                          ...
                          uint32_t channels) { h3_cuda_seterr(gpu); return 0; }
int h3_gpu_snake1d_f32(h3_gpu *gpu, h3_gpu_tensor *output,
                       const h3_gpu_tensor *input,
                       const h3_gpu_tensor *alpha, uint32_t batch,
                       uint32_t length, uint32_t channels) { h3_cuda_seterr(gpu); return 0; }
...
int h3_gpu_conv3d_f32(h3_gpu *gpu, h3_gpu_tensor *output,
                      ...
                      uint32_t stride_height, uint32_t stride_width) { h3_cuda_seterr(gpu); return 0; }
int h3_gpu_vae_encoder_group_norm_silu_f32(
                      ...
                      const h3_gpu_tensor *bias, uint32_t batch,
                      uint32_t depth, uint32_t height, uint32_t width,
                      uint32_t channels, uint32_t groups, float epsilon) { h3_cuda_seterr(gpu); return 0; }
...
int h3_gpu_mlp_bf16(h3_gpu *gpu, h3_gpu_tensor *output,
                    ...
                    uint32_t output_dim) { h3_cuda_seterr(gpu); return 0; }
...
int h3_gpu_mlp_int8_bf16(h3_gpu *gpu, h3_gpu_tensor *output,
                         ...
                         int input_is_quantized) { h3_cuda_seterr(gpu); return 0; }
...
int h3_gpu_gelu_bf16(h3_gpu *gpu, h3_gpu_tensor *output,
                     const h3_gpu_tensor *input, uint32_t elements,
                     int approximate) { h3_cuda_seterr(gpu); return 0; }
int h3_gpu_vision_qkv_rope_bf16(
                     ...
                     const h3_gpu_tensor *rope_sin, uint32_t sequence,
                     uint32_t heads, uint32_t head_dim,
                     uint32_t rope_half) { h3_cuda_seterr(gpu); return 0; }
...
int h3_gpu_embedding_bf16(h3_gpu *gpu, h3_gpu_tensor *output,
                          ...
                          uint32_t vocab_size, uint32_t width) { h3_cuda_seterr(gpu); return 0; }
...
int h3_gpu_head_rms_norm_bf16(h3_gpu *gpu, h3_gpu_tensor *tensor,
                              const h3_gpu_tensor *weight,
                              uint32_t sequence, uint32_t heads,
                              uint32_t head_dim, float epsilon) { h3_cuda_seterr(gpu); return 0; }
int h3_gpu_rope_text_bf16(h3_gpu *gpu, h3_gpu_tensor *query,
                          h3_gpu_tensor *key,
                          const h3_gpu_tensor *rope_cos_f32,
                          const h3_gpu_tensor *rope_sin_f32,
                          uint32_t sequence, uint32_t query_heads,
                          uint32_t kv_heads, uint32_t head_dim) { h3_cuda_seterr(gpu); return 0; }
int h3_gpu_gqa_causal_bf16(h3_gpu *gpu, h3_gpu_tensor *output,
                           ...
                           uint32_t sequence, uint32_t query_heads,
                           uint32_t kv_heads, uint32_t head_dim,
                           float scale) { h3_cuda_seterr(gpu); return 0; }
...
int h3_gpu_token_expand_delta_bf16(
                           ...
                           float update_scale) { h3_cuda_seterr(gpu); return 0; }
...
int h3_gpu_euler_bf16(h3_gpu *gpu, h3_gpu_tensor *sample,
                      ...
                      float delta, float ratio) { h3_cuda_seterr(gpu); return 0; }
int h3_gpu_silu_mul_bf16(h3_gpu *gpu, h3_gpu_tensor *output,
                         const h3_gpu_tensor *gate,
                         const h3_gpu_tensor *up, uint32_t elements) { h3_cuda_seterr(gpu); return 0; }
